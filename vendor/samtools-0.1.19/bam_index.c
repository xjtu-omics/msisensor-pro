...
bam_index_t *bam_index_load_local(const char *_fn) {
    ...
    if (fp == 0) { // try "{base}.bai"
        ...
        // added By Peng Jia
        // Remove the condition to avoid error reading of {base}.bai index
//        if (s == fn + strlen(fn) - 3) {
        strcpy(fnidx, fn);
        fnidx[strlen(fn) - 1] = 'i';
        fp = fopen(fnidx, "rb");
...
pair64_t *
get_chunk_coordinates(const bam_index_t *idx, int tid, int beg, int end, int *cnt_off) { // for pysam compatibility
    bam_iter_t iter;
    pair64_t *off;
