...
RefSeq::RefSeq() :
		totalHomosites(0), totalMicrosates(0), totalSites(0) {
	// buffer resize
...
bit8_t RefSeq::LoadOneSite(const std::string & chr, const std::string & seq,
		int loc, bit8_t hLen, bit16_t type, bit16_t len, HomoSite & oneSite) {
	...
	for (unsigned char s = 0; s < param.ContextLength; s++) {
		p0 = h + contextStart + s;
		p1 = h + loc + len * hLen + s;
		if ((alphabet[*p0] == 4) || (alphabet[*p1] == 4)) {
			return 0;
		}
		flankH <<= 2;
		flankH |= alphabet[*p0];
...
void RefSeq::TestSitesBinary() {
	std::cout << "chromosome" << "\t" << "location" << "\t" << "sitelength"
			<< "\t" << "sitecontent" << "\t" << "repeattimes" << "\t"
			<< "front_flank" << "\t" << "tail_flank" << std::endl;
	for (int i = 0; i < homosBuffer.size(); i++) {
		std::cout << homosBuffer[i].chr << "\t" << homosBuffer[i].location
				<< "\t" << int(homosBuffer[i].typeLen) << "\t"
				<< homosBuffer[i].homoType << "\t" << homosBuffer[i].length
				<< "\t" << homosBuffer[i].frontKmer << "\t"
			...
			for (k = 2; k <= param.MaxMicrosate; k++) {
				...
				if (hitLength >= param.Repeats) {
					...
					if (param.HomoOnly == 0) {
						// load one microsate
						HomoSite toneSite;
						if (LoadOneSite(name, _seq, i0, k, s1, hitLength,
								toneSite)) {
							homosBuffer.push_back(toneSite);
							totalMicrosates++;
							totalSites++;
