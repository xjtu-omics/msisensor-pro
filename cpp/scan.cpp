...
void ScanUsage(void) {
	std::cerr << "\nUsage:  msisensor-pro scan [options] \n\n"
			...
			<< "       -l   <int>      minimal homopolymer(repeat unit length = 1) size, default="
			<< param.MininalHomoSize << "\n"
			<< "       -c   <int>      context length, default="
			<< param.ContextLength << "\n"
			<< "       -m   <int>      maximal homopolymer size [5-32], default="
			<< param.MaxHomoSize << "\n"
			<< "       -s   <int>      maximal length of microsatellite [1-32] , default="
			<< param.MaxMicrosate << "\n"
			<< "       -r   <int>      minimal repeat times of microsatellite(repeat unit length>=2), default="
			<< param.Repeats << "\n"
			<< "       -p   <int>      output homopolymer only, 0: no; 1: yes, default="
			<< param.HomoOnly << "\n" << "       \n" << "       -h   help\n\n"
			<< "Function: \n"
...
int mGetOptions(int rgc, char *rgv[]) {
	...
	for (i = 1; i < rgc; i++) {
		...
		case 'o':
			...
			break;
		case 'l':
			param.MininalHomoSize = atoi(rgv[++i]);
			break;
		case 'c':
			param.ContextLength = atoi(rgv[++i]);
			break;
		case 'm':
			param.MaxHomoSize = atoi(rgv[++i]);
			break;
		case 's':
			param.MaxMicrosate = atoi(rgv[++i]);
			break;
		case 'r':
			param.Repeats = atoi(rgv[++i]);
			break;
		case 'p':
			param.HomoOnly = atoi(rgv[++i]);
			break;
			break;
		case 'h':
			ScanUsage();
