...
HomoSite::HomoSite() :
		homoType(0), length(0), frontKmer(0), endKmer(0), typeLen(0), chr(""), transfer(
				""), bases(""), fbases(""), ebases(""), location(0), lowcut(0), highcut(
				0), normalDis(NULL), tumorDis(NULL)
		////////////
				, normalCov(0), tumorCov(0), withSufCov(false), normalWithSufCov(
				false), dif(0.0), pValue(0.0), comentropy(0.0), hunterValueU(
				0.0) // add by Yelab
				, hunterValueV(0.0) // add by Yelab
				, somatic(false), withGenotype(false), thres(
...
void HomoSite::PouroutDis(Sample &sample) {
	sample.outputDistribution << chr << " " << location << " " << fbases << " "
			<< length << "[" << bases << "] " << ebases << "\n";
...
void HomoSite::DisTumorSomatic(Sample &sample) {
	...
	if (withSufCov)
		sample.numberOfDataPoints++;
	if (reportSomatic) {
		sample.outputSomatic << chr << "\t" << location << "\t" << fbases
				<< "\t" << length << "\t" << bases << "\t" << ebases;
...
void HomoSite::HunterTrain(Sample &sample) {        //add by yelab
	...
	if (CovLargeNum / polyscan.totalBamNormalsNum >= paramd.NormalcovCutoff) {
		...
		sample.outputTrain << chr << "\t" << location << "\t" << fbases << "\t"
				<< length << "\t" << bases << "\t" << ebases;
...
//			std::cout<<site<<"\n";
		...
		if (withSufCov) {
			sample.numberOfDataPoints++;
			sample.outputAll << chr << "\t" << location << "\t" << fbases
					<< "\t" << length << "\t" << bases << "\t" << ebases << "\t"
					<< std::fixed << tumorCov << "\t" << std::fixed
					<< hunterValueU << "\t" << std::fixed << hunterValueU
					<< std::endl;
...
//			}
		...
		if (withSufCov) {
			...
			sample.outputAll << chr << "\t" << location << "\t" << fbases
					<< "\t" << length << "\t" << bases << "\t" << ebases << "\t"
					<< std::fixed << hunterValueU << "\t" << std::fixed
					<< hunterValueV << "\t" << std::fixed << tumorCov << "\t"
					<< std::fixed << thres << "\n";
		}	//YeLab
...
// distance
double HomoSite::DistanceBetweenTwo(unsigned short * FirstOriginal,
	...
	// normalization
	for (unsigned i = 0; i < dispots; i++) {
		if (FirstOriginal[i] < 1)
			FirstNormalized[i] = 0.0;
		else
			FirstNormalized[i] = (FirstOriginal[i] / (double) sumFirst);
		if (SecondOriginal[i] < 1)
			SecondNormalized[i] = 0.0;
		else
			SecondNormalized[i] = SecondOriginal[i] / (double) sumSecond;
	}
	...
	for (unsigned i = 0; i <= dispots; i++) {
		...
		if (Min[i] < SmallDouble)
			Min[i] = 0.0;
		if (Max[i] < SmallDouble)
			Max[i] = 0.0;
	}
	...
	if (AreaMax < AreaMin) {
		std::cerr << "something is wrong with distance calculation " << AreaMax
				<< " " << AreaMin << std::endl;
		for (unsigned index = 0; index < 100; index++) {
			std::cerr << index << "\t" << FirstOriginal[index] << "\t"
					<< SecondOriginal[index] << "\t" << sumFirst << "\t"
					<< sumSecond << "\t" << FirstNormalized[index] << "\t"
					<< SecondNormalized[index] << "\t" << Min[index] << "\t"
					<< Max[index] << std::endl;
		}
	}
	delete[] Min;
...
std::vector<double> HomoSite::Hunterp(unsigned short * tumorDis,
		unsigned int dispots, unsigned int reflen) {
	int i;
...
void HomoSite::ComputeGenotype(unsigned short * NormalReadCount) {
	...
	Offset = 1;
	CoverageCutoff = paramd.covCutoff;
	first = second = Sum = 0;

	// find the largest number
	for (unsigned int pos_index = 0; pos_index < paramd.s_dispots;
			pos_index++) {
		// NormalReadCount
