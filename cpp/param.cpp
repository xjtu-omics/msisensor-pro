...
Param::Param() :
		bufSize(500000), ncpu(1), chains(0), max_dbseq_size(300000000) //300Mb
				, append_dbseq_size(0x1000000) //16Mb
				, MininalHomoSize(10) //5->10
				, ContextLength(5), MaxHomoSize(50), SpanSize(500), MininalHomoForDis(
				10)
				// microsate
				, MinMicrosate(3), MinMicrosateForDis(5), MaxMicrosateForDis(40)

		, DisSpan(500), HomoOnly(0), MicrosateOnly(0), s_dispots(100), MaxMicrosate(
				5), Repeats(5) //3->5
				, numberThreads(1), PercentPairs(20), PercentPairsNumber(2), HomoCoverage(
				40), windowSize(500000) // window size (default 50k)
				, covCutoff(1), Normalization(0), fdrThreshold(0.05), comentropyThreshold(
				1) // for tumor only
				, hunterThreshold(0.1) //YeLab
				, outputzeroDis(0) //Yelab
				, NormalcovCutoff(0.5) //Yelab
				, train(false), pro(false), hard(false), sampleRatio(0.5) {
	inital_homo_phabet();
	initalphabet();
	initrevalphabet();
}
